...
int main(int argc, char** argv)
{
	...
	string execFileName(argv[0]);
        cout << "Usage: " << execFileName.substr(execFileName.find_last_of('/') + 1, execFileName.length()) << " [file path to jpg image]" << endl;
        cout << "or attempt to open camera stream #0 if no file path is provided." << endl;
	exit(-1);
    } 
    else if (argc == 2) 
    {
	...
	if (pos != string::npos)
	{
	    outputFileName = inputFileName.substr(0, pos) + '_' + outputFileName;
	}
    }
    else 
    {
	cout << "Opening #0 camera stream ..." << endl;
        cap.open(0);
    }
    int padding = 20;
    while(waitKey(1) < 0) {
      ...
      {
	  cout << "Opening " << inputFileName << " file ..." << endl;
          frame = imread(inputFileName);
      }
      if (frame.empty())
      {
	  cout << "No frame was read, please press any key to exit." << endl;
          waitKey();
          break;
      }
	...
	cout << "Writing output to " << outputFileName << " file." << endl;
        imwrite(outputFileName,frameFace);
      }
      if (!inputFileName.empty())
      {
          cout << "Finished processing " << inputFileName << " file. Please press any key to exit." << endl;
	  waitKey();
	  break;
      }
    }
